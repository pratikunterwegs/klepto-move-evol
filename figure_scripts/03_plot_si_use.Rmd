...
```{r }
data_pref = fread("data_sim/results/data_syndrome_by_strategy.csv")

data_pref[, strategy := fifelse(sim_type == "obligate", klept_bias, "consumer")]

data_pref = data_pref[handler_pref == T, list(
  handler_pref = mean(prop_handler_pref)
), by = c("sim_type", "rep", "strategy", "gen")]
```

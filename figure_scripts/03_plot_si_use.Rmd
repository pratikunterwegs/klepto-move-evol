...
```{r }
# read back in
data <- fread("data_sim/results/data_sparse_weight_evolution.csv")
...
wt_handler <- wt_handler[, list(pref_handlers = sum(weight_prop)),
  by = c("sim_type", "rep", "regrowth", "gen")
]
...
# HANDLE SCENARIO 2
wt_handler_s2 = fread("data_sim/results/data_syndrome_by_strategy.csv")
wt_handler_s2 = wt_handler_s2[sim_type == "obligate",]
wt_handler_s2 = wt_handler_s2[regrowth == 0.01, list(N = sum(N)),
                              by = c("sim_type", "rep", "regrowth", 
                                     "klept_bias", "handler_pref", "gen")
                              ]

# binarise handler preference to positive or negative
wt_handler_s2[, c("klept_bias", "handler_pref") := list(
  factor(ifelse(klept_bias, "forager", "klept")),
  factor(ifelse(handler_pref > 0, "prefers handlers", "avoids handlers"),
         levels = c("prefers handlers", "avoids handlers")
  )
)]

# count by comp and si strategy
wt_handler_s2 = wt_handler_s2[, list(N = sum(N)),
  by = c("gen", "klept_bias", "handler_pref", "regrowth", "rep", "sim_type")
]
wt_handler_s2[, prop_per_strat := N / sum(N), 
  by = c("gen", "klept_bias", "regrowth", "rep")
]
  ...
  ggplot() +
    ...
    geom_path(
      ...
      aes(gen, pref_handlers,
        col = strategy,
        group = interaction(rep, strategy)
      )
    )+
    scale_colour_manual(
      values = col_vals,
    ...
    guides(
      colour = guide_legend(nrow = 1, ncol = 4)
    )
})

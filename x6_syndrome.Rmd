...
## Item preference by strategy

```{r}
# get total agents per klept bias and item pref (4 classes total)
data_item_pref = copy(data)
data_item_pref = data_item_pref[, .(totalN = sum(N)), 
               by = c("gen", "replicate", "regrowth",
                             "klept_bias", "nh_pref")]
data_item_pref = data_item_pref[, prop := totalN / sum(totalN), 
                      by = c("gen", "replicate", "regrowth", "klept_bias")]
```

```{r}
ggplot(data_item_pref[regrowth < 0.02 & gen < 50])+
  geom_line(aes(gen, prop,
                col = interaction(klept_bias, nh_pref),
                group = interaction(klept_bias, nh_pref, replicate)),
             size = 1)+
  facet_grid(~regrowth)
```

...
## Functional response of intake to items

```{r}
data <- fread("data/results/data_funresp_intake_items.csv")
...
# as well as sim type
data_intake_items <- data[variable != "pc_intake_total", 
...
# convert to number
data_intake_items[, item_num := get_lower(items)]

# scale by max items
data_intake_items[, item_num := scales::rescale(
  item_num
),
by = c("sim_type", "regrowth")]
```
...
```{r}
# split and plot
data_intake_items <- split(data_intake_items, by = "sim_type")

subfig_intake_items <- 
  lapply(data_intake_items, function(df){
  ggplot(df)+
  geom_point(aes(item_num, value.median,
                 col = variable),
             size = 0.2,
             shape = 1,
             show.legend = F)+
  ...
  # scale_x_sqrt()+
  # scale_y_sqrt()+
  facet_grid(sim_type ~ regrowth, scales = "fixed")+
...
# collect figures
fig_intake_items <- 
  wrap_plots(subfig_intake_items[c(
  "foragers",
  "obligate",
  "facultative"
)],
...
```{r}
ggsave(fig_intake_items,
  filename = "figures/fig_intake_items.png",
  dpi = 150
)
```
...
## Aggregative response: Agents ~ items

```{r}
data = fread("data/results/data_funresp_predators_items.csv")
# get specific regrowths
data <- data[regrowth %in% c(0.001, 0.01, 0.05, 0.1, 0.25), ]

# summarise
data_predators_prey = data[variable != "total_agents", 
                           unlist(lapply(.SD, multsum), recursive = F),
                           .SDcols = c("value"),
                           by = c("sim_type", "regrowth",
                                  "items", "variable")]
...
# get lower
data_predators_prey[, item_num := get_lower(items)]

# scale item num
data_predators_prey[, item_num_scaled := scales::rescale(item_num),
                    by = c("sim_type", "regrowth")]

# drop NA
data_predators_prey <- na.omit(data_predators_prey)
```

```{r}
ggplot(data_predators_prey)+
  geom_jitter(aes(item_num_scaled, value.median,
                 col = variable),
              alpha = 0.5,
             size = 0.2, shape = 1)+
  facet_grid(sim_type ~ regrowth)+
...
ggsave(filename = "figures/fig_aggegrative_response.png")
```

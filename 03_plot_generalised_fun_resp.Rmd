...
```{r}
# split the data
data_model <- split(data, by = c("sim_type", "regrowth", 
                                 "variable"))
```
...
# fig_coef_intake <- 
  ...
  facet_grid(sim_type~vbl, scales = "free")+
  scale_colour_manual(
...
### Relative effect of individuals and items

```{r}
# cast values
sim_coefs_2 <- dcast(
  sim_coefs,
  formula = sim_type + regrowth + strategy ~ vbl, 
  value.var = "cf"
)

# get relative importance
sim_coefs_2[, rel_imp := abs(items) - abs(individuals)]
```

```{r}
ggplot(sim_coefs_2)+
  geom_point(aes(regrowth, rel_imp,
                 col = strategy))+
  geom_line(aes(regrowth, rel_imp,
                 col = strategy))+
  facet_grid(~ sim_type)+
  xlim(NA, 0.075)+
  labs(x = "r_base (regrowth rate)",
       y = "rel. importance (coef. items - coef. indiv.)")

ggsave(filename = "figures/fig_relative_importance.png")
```

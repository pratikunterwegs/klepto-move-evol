...
ggplot(data_equi)+
  geom_boxplot(aes(factor(regrowth), value, col = sim_type,
                   group = interaction(sim_type, regrowth)),
               outlier.size = 0)+
  scale_colour_manual(
    values = qualitative_hcl(3, "Set2", l = 50, rev = T)
  )

# boxplot fitness
fig_strategy_fitness = ggplot(data_equi)+
  geom_jitter(aes(factor(regrowth), 
                  pop_fitness, col = sim_type,
                   group = interaction(sim_type, regrowth)),
              size = 0.1, alpha = 0.2,
              shape = 1,
              # width = 0.05,
              position = position_dodge(width = 0.75))+
  geom_boxplot(aes(factor(regrowth), pop_fitness, col = sim_type),
               outlier.shape = NA)+
  scale_colour_manual(
    values = qualitative_hcl(3, "Dynamic", l = 50, rev = T)
  )+

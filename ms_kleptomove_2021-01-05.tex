...
\paragraph*{\textbf{WORK IN PROGRESS}}
...
\subsection*{Mechanisms of an Ideal Dynamic Distribution}

In all three scenarios, per-capita intake rates were roughly equalised across cell qualities (Figure X).
This finding is broadly in the spirit of the original IFD concept that individuals should distribute themselves in such a way as to be unable to obtain a better intake by moving; this is despite ignoring all the IFD assumptions.
%%
Since the IFD is an emergent phenomenon of the population-landscape interaction, individuals redistribute themselves when the underlying landscape changes \citep[][]{cressman2006}.
Individuals following the producer strategy navigate this complex landscape by using indirect cues of the immediate benefit as well as long-term quality of landscape cells.
This mechanism is very poorly captured by phenomenological models of functional response coefficients for individuals and prey-items.
%%
...
These models would predict that individuals do not select for prey-items, and that individuals should move away from each other at low regrowth rates, while moving towards each other at higher regrowth rates
%%
However, individuals are not neutral to prey-items, instead showing a strong preference at all regrowth rates.
Further, individuals' response to other individuals is based on behavioural state; populations evolve to move towards handlers and away from non-handlers across a range of regrowth rates.
...
This social-information use may explain why animal groups are apparently larger than predicted on low-productivity landscapes without invoking extrinsic mechanisms such as predation risk.
%%

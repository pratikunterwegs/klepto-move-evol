...
# write file
fwrite(data_summary, file = "data_sim/results/data_funresp_agents.csv")
```
...
# write to file
fwrite(data_summary, file = "data_sim/results/data_funresp_strategy.csv")
```
...
# write to file
fwrite(data_summary, file = "data_sim/results/data_funresp_intake_items.csv")
```
...
```{r}
setDT(data_summary)
data_summary_2 <- data_summary[, list(
  meanv = mean(value),
  medv = median(value),
  sdv = sd(value)
),
...
```
```{r}
data_summary_3 <- data_summary_3[, list(
  meanv = mean(meanv),
  medv = median(meanv),
  sdv = sd(meanv)
),

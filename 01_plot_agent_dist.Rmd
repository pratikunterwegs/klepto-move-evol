...
```{r}
# remove random
data <- data[sim_type != "random" & regrowth == 0.1, ]
...
subplots <- 
  Map(function(df) {
    ...
    if (!unique(df$layer)[[1]] == "items") {
      pal <- c(sequential_hcl(1, "Reds", rev = F),
               sequential_hcl(1, "PuBu", rev = F))
    } else {
      pal <- sequential_hcl(1, "Greens 2", rev = F)
    }
    
    ggplot(df)+
      geom_point(aes(cap, med_val,
                          # ymin = med_val - sd_val,
                          # ymax = med_val + sd_val,

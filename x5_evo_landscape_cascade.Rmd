...
subplot_land <- lapply(landscape, function(df) {
  ...
  ggplot(df) +
    ...
    scale_fill_gradientn(
      colours = pal,
      na.value = ifelse(unique(df$variable) == "items", "white", "grey95"),
      limits = c(0.7, NA)
    ) +
...
# make figure 6
figure_8 <-
    (subplot_land[["foragers.gradient"]] + subplots[["foragers"]]) /
    (subplot_land[["obligate.gradient"]] + subplots[["obligate"]]) /
    (subplot_land[["facultative.gradient"]] + subplots[["facultative"]]) +
  plot_annotation(
      tag_levels = c("A")
  )

# why is this not compatible with lapply, or anything similar!
figure_8[[1]] = figure_8[[1]] +
    plot_layout(guide = "collect",
                tag_level = "new")

figure_8[[2]] = figure_8[[2]] +
    plot_layout(guide = "collect",
                tag_level = "new")

figure_8[[3]] = figure_8[[3]] +
    plot_layout(guide = "collect",
                tag_level = "new")

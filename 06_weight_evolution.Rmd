...
fwrite(data, file = "data/data_weight_evolution.csv")
```

## Prepare plots

```{r}
data2 <- data[regrowth == 0.05 & sim_type == "obligate", ]

# get label weights
label_wt <- c(
  sprintf("weight: %i", 2:8)
)
names(label_wt) <- c(2:8)
...
plots <- Map(function(le) {
  ...
  subfigures <- lapply(le, function(df) {
    ...
    # get plot
    ggplot(df)+
      geom_tile(aes(gen, weight_value, 
                    fill = weight_prop),
                show.legend = FALSE)+
